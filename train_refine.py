...
def main(rank):
    ...
    dataset_val = create_dataset(opt, mode=opt.val_epoch_split, shuffle=False)  # opt.val_epoch_split = test_train
    dataset_iterval = create_dataset(opt, mode=opt.val_split, shuffle=False)    # opt.val_split = val
    ...
    for epoch in range(current_epoch + 1, opt.n_epochs + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        if opt.is_master and epoch % opt.val_epoch_freq == 0:
            ...
            model.save_best_networks(val_losses['psnr'])
